...
Based on the generated sample $\{\ibx_i^{(k)} \}_{i=1}^n$, we can compute the MLEs $\{\hbB^{(r)}, \hbSi^{(r)}, \hla^{(r)}\}$ for the four specific distributions of the Ge-N$_2$ family via the N-EM algorithm, calculate four KL divergence values $\{\tsf{KL}_1^{(r)}, \dots, \tsf{KL}_4^{(r)}\}$, ISE values $\{\tsf{ISE}_1^{(r)}, \dots, \tsf{ISE}_4^{(r)}\}$ and MSE values $\{\tsf{MSE}_1^{(r)}, \dots, \tsf{MSE}_4^{(r)}\}$. Then we sort them to obtain the corresponding ranks. Finally, the average KL divergences, average ISE, average MSE and the average ranks (in parentheses) are reported in Figure \ref{zyf.four.fig2}.

\begin{figure}[H]
    \centering
    \includegraphics[width=1.0\linewidth]{figures/plot17.png}
    \caption{Model comparisons based on $K=1$,000 replications for Cases (C$_1$) and (C$_2$).}
    \label{zyf.four.fig2}
\end{figure}
