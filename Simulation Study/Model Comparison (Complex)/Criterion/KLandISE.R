KLandISE <- function(X_test, Z_test, para_true, para_est) {
  ...
  log_f_hat <- matrix(0, nrow = N_test, ncol = 6)
  colnames(log_f_hat) <- c("GaN", "IGaN", "IGauN", "RIGauN", "Norm", "NPMN")
  ...
  for (i in 1:N_test) {
    ...
    # 2.5 Norm
    f_Norm <- Norm.density(x_i, z_i, para_est$Norm$Beta, para_est$Norm$Sigma)
  ...
  ISE_results <- numeric(6)
  names(ISE_results) <- c("GaN", "IGaN", "IGauN", "RIGauN", "Norm", "NPMN")
