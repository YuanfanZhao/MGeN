...
Simulation <- function(number, size, Beta, Sigma, Kurtosis, Ratio) {
  ...
  # 评价指标
  KL.value <- matrix(nrow = number, ncol = 6)
  ISE.value <- matrix(nrow = number, ncol = 6)
  MSE.value <- matrix(nrow = number, ncol = 6)
  M.value <- matrix(nrow = number, ncol = 3)
  Rank.value <- array(dim = c(3, 6, number))
  ...
  while(iter <= number) {
    ...
    NPMN.fit <- tryCatch({
      NPMN.MLE(X.train, Z.train, Beta, s, 10)
    }, error = function(e) {
      NA
    }, warning = function(w) {
      NA
    ...
    if (!all(is.na(GaN.fit)) & !all(is.na(IGaN.fit)) & !all(is.na(IGauN.fit)) & !all(is.na(RIGauN.fit)) & !all(is.na(NPMN.fit))) {
      if (GaN.fit$index == TRUE & IGaN.fit$index == TRUE & IGauN.fit$index == TRUE & RIGauN.fit$index == TRUE & NPMN.fit$index == TRUE) {
        ...
        estimated_parameters <- list(
          ...
          Norm = list(Beta = Norm.fit$Beta, Sigma = Norm.fit$Sigma),
