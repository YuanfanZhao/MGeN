...
Simulation <- function(number, size, Mu, Sigma, Lambda, B = 2500, G = 10000, 
  ...
  while (k <= number) {
    ...
    if (!all(is.na(Res))) {
      if (Res$index == TRUE) {
        Mode.fit <- tryCatch({
          GaN.ECM(X, m, s, Res$Lambda, Mu0 = Mu0, Kappa0 = Kappa0, Lambda0 = Lambda0, Nu0 = Nu0)
        }, error = function(e) {
          NA
        }, warning = function(w) {
          NA
        ...
        if (!all(is.na(Mode.fit))) {
          if (Mode.fit$index == TRUE) {
            Mean.fit <- GaN.DA(X, m, s, Res$Lambda, Mu0 = Mu0, Kappa0 = Kappa0, Lambda0 = Lambda0, Nu0 = Nu0, B = B, G = G)
