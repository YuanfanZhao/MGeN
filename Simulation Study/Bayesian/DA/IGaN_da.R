IGaN.DA <- function(X, Mu, Sigma, Lambda, Mu0, Kappa0, Lambda0, Nu0, B, G) {
  ...
  while (iter <= G) {
    # I-step: 生成潜在变量
    A <- apply(X, 1, function(x) as.numeric(t(x - Mu.value[iter, ]) %*% solve(Sigma.value[, , iter]) %*% (x - Mu.value[iter, ])))
    tau <- sapply(A, function(a) rgig(1, d / 2 - Lambda, 2 * Lambda, a))
