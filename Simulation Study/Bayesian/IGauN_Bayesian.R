...
Simulation <- function(number, size, Mu, Sigma, Lambda, Prior = "Non-information", B = 2500, G = 10000) {
  ...
  # 导入ECM算法、DA算法
  source("ECM/IGauN_ecm.R")
  source("DA/IGauN_da.R")
  ...
  while (k <= number) {
    ...
    if (!all(is.na(Res))) {
      if (Res$index == TRUE) {
        Mode.fit <- tryCatch({
          IGauN.ECM(X, m, s, Res$Lambda, Mu0, Kappa0, Lambda0, Nu0)
        }, error = function(e) {
          NA
        }, warning = function(w) {
          NA
        ...
        if (!all(is.na(Mode.fit))) {
          if (Mode.fit$index == TRUE) {
            Mean.fit <- IGauN.DA(X, m, s, Res$Lambda, Mu0, Kappa0, Lambda0, Nu0, B, G)
