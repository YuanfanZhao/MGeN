...
Simulation <- function(number, size, Beta, Sigma, Lambda) {
  ...
  rIGaN <- function(n, Z, Beta, Sigma, Lambda) {
    
    # 位置参数μ（n×d）
    Mu <- Z %*% Beta
    
    # 生成τ分布的随机数
    tau <- 1 / rgamma(n, Lambda, Lambda)
  ...
  IGaN.density <- function(x, z, Beta, Sigma, Lambda) {
    d <- ncol(Beta)
  ...
  IGaN.loglikeli <- function(X, Z, Beta, Sigma, Lambda) {
    d <- ncol(Beta)
    
    compute.density <- function(data) {
      x <- data[1:d]
      z <- data[-(1:d)]
      IGaN.density(x, z, Beta, Sigma, Lambda)
    }
  ...
  IGaN.MLE <- function(X, Z, Beta_new, Sigma_new, Lambda_new, max_iter = 1000, ep = 1e-8) {
    index <- FALSE
    iter <- 1
  ...
  while(k <= number) {
    ...
    X <- rIGaN(size, Z, Beta, Sigma, Lambda)
    
    
    Res <- tryCatch({
      IGaN.MLE(X, Z, Beta, Sigma, Lambda)
    }, error = function(e) {
      NA
    }, warning = function(w) {
      NA
