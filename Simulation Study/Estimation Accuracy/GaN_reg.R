...
Simulation <- function(number, size, Beta, Sigma, Lambda) {
  ...
  rGaN <- function(n, Z, Beta, Sigma, Lambda) {
  ...
  GaN.loglikeli <- function(X, Z, Beta, Sigma, Lambda) {
    d <- ncol(Beta)
    
    compute.density <- function(data) {
      x <- data[1:d]
      z <- data[-(1:d)]
      GaN.density(x, z, Beta, Sigma, Lambda)
    }
  ...
  GaN.MLE <- function(X, Z, Beta_new, Sigma_new, Lambda_new, max_iter = 1000, ep = 1e-8) {
    index <- FALSE
    iter <- 1
  ...
  while(k <= number) {
    ...
    X <- rGaN(size, Z, Beta, Sigma, Lambda)
    
    
    Res <- tryCatch({
      GaN.MLE(X, Z, Beta, Sigma, Lambda)
    }, error = function(e) {
      NA
    }, warning = function(w) {
      NA
