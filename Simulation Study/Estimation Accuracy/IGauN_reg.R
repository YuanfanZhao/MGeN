...
Simulation <- function(number, size, Beta, Sigma, Lambda) {
  ...
  rIGauN <- function(n, Z, Beta, Sigma, Lambda) {
    
    # 位置参数μ（n×d）
    Mu <- Z %*% Beta
    
    # 生成τ分布的随机数
    tau <- rinvgauss(n, mean = Lambda / (Lambda - 1), shape = Lambda)
  ...
  IGauN.density <- function(x, z, Beta, Sigma, Lambda) {
    d <- ncol(Beta)
  ...
  IGauN.MLE <- function(X, Z, Beta_new, Sigma_new, Lambda_new, max_iter = 1000, ep = 1e-8) {
    index <- FALSE
    iter <- 1
    ...
    Loglikeli_new <- IGauN.loglikeli(X, Z, Beta_new, Sigma_new, Lambda_new)
  ...
  while(k <= number) {
    ...
    X <- rIGauN(size, Z, Beta, Sigma, Lambda)
    
    
    Res <- tryCatch({
      IGauN.MLE(X, Z, Beta, Sigma, Lambda)
    }, error = function(e) {
      NA
    }, warning = function(w) {
      NA
