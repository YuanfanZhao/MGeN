...
Simulation <- function(number, size, Mu, Sigma, Lambda) {
  ...
  RIGauN.density <- function(x, Mu, Sigma, Lambda) {
    d <- length(Mu)
  ...
  RIGauN.loglikeli <- function(X, Mu, Sigma, Lambda) {
    # 返回密度函数的对数和
    return(sum(log(apply(X, 1, RIGauN.density, Mu = Mu, Sigma = Sigma, Lambda = Lambda))))
  }
  
  RIGauN.MLE <- function(X, Mu_new, Sigma_new, Lambda_new, max_iter = 1000, ep = 1e-8) {
    index <- FALSE
    iter <- 1
    ...
    Loglikeli_new <- RIGauN.loglikeli(X, Mu_new, Sigma_new, Lambda_new)
  ...
  while(k <= number) {
    ...
    if (!all(is.na(Res))) {
      if (Res$index == 1) {
        ...
        result_moment[[2]][, , k] <- s
        result_moment[[3]][k] <- l
        result_moment[[4]][k] <- RIGauN.loglikeli(X, m, s, l)
